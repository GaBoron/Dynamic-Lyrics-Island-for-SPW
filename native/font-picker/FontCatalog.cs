...
internal static class FontCatalog
{
    ...
    public static IReadOnlyList<FontOption> Load()
    {
        ...
        {
            new("", "内置 Noto Sans SC", "插件默认 · 多语言缺字回退",
    ...
    [
        BundledFace("NotoSansSC-Thin.otf", "极细 100", 100),
        BundledFace("NotoSansSC-Light.otf", "细体 300", 300),
        BundledFace("NotoSansSC-DemiLight.otf", "次细 350", 350),
        BundledFace("NotoSansSC-Regular.otf", "常规 400", 400),
        BundledFace("NotoSansSC-Medium.otf", "中等 500", 500),
        BundledFace("NotoSansSC-Bold.otf", "粗体 700", 700),
        BundledFace("NotoSansSC-Black.otf", "极粗 900", 900)
    ];

    // Every bundled weight ships as its own file; the preview must reference that file to show it.
    private static FontFace BundledFace(string file, string displayName, int weight) =>
        new("", displayName, weight, false, new FontFamily($"ms-appx:///Assets/{file}#Noto Sans SC"));
